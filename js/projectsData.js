...
const projectsData = [
    {
        ...
        heroType: "image",
        heroImage: "https://images.unsplash.com/photo-1550745165-9bc0b252726f?ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=80",
        heroAlt: "Cyberpunk City",
        badge: "Unity 6",
...
function projectCardTemplate(project, index) {
    ...
    if (project.heroType === "image") {
        heroHTML = `
            <div class="h-card-3d bg-slate-900 relative">
                <img src="${project.heroImage}" alt="${project.heroAlt}"
                     class="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity">
                <div class="absolute top-4 right-4 ${project.badgeStyle} px-2 py-1 rounded text-xs font-mono">
                    ${project.badge}</div>
            </div>`;
    } else if (project.heroType === "model-viewer") {
        heroHTML = `
            <div class="h-card-3d bg-slate-900 relative">
    ...
    } else if (project.heroType === "icon") {
        heroHTML = `
            <div class="h-card-3d bg-slate-900 relative flex items-center justify-center">
                <i data-lucide="${project.icon}"
                   class="w-20 h-20 ${project.iconColor} ${project.iconHoverClass || ''}"></i>
            </div>`;
    }
